...
`);const m=[];s=s.replace(/\$\$([\s\S]*?)\$\$/g,(x,h)=>{const N=m.length;return m.push({type:"display",content:h}),` @@MATH_BLOCK_${N}@@ `}),s=s.replace(/\\begin\{aligned\}([\s\S]*?)\\end\{aligned\}/g,(x,h)=>{const N=m.length;return m.push({type:"display",content:`\\begin{aligned}${h}\\end{aligned}`}),` @@MATH_BLOCK_${N}@@ `}),s=s.replace(new RegExp("(?<!\\\\)\\$([^$\\n]+?)\\$","g"),(x,h)=>{const N=m.length;return m.push({type:"inline",content:h}),` @@MATH_BLOCK_${N}@@ `});let y=Be.parse(s).replace(/<h3>(.*?)<\/h3>/g,(x,h)=>`<h3 id="${"topic-"+h.replace(/<[^>]*>/g,"").trim().replace(/\s+/g,"-").toLowerCase()}">${h}</h3>`);const q=Me.sanitize(y,{USE_PROFILES:{html:!0,mathml:!0},ADD_TAGS:["math","annotation","semantics","mrow","msub","msup","msubsup","mover","munder","munderover","mmultiscripts","mprec","mnext","mtable","mtr","mtd","mfrac","msqrt","mroot","mstyle","merror","mpadded","mphantom","mfenced","menclose","ms","mglyph","maligngroup","malignmark","maction","svg","path","use","span","div"],ADD_ATTR:["id","target","xlink:href","class","style","aria-hidden","viewBox","d","fill","stroke","stroke-width","data-filename","encoding","display"]}),A=[...a.examples||[]].sort((x,h)=>h.filename.length-x.filename.length);let M=q;return A.forEach(x=>{const h=x.filename.replace(".","\\."),N=new RegExp(`(?<!['".\\w])(${h})(?!['".\\w])`,"g");M=M.replace(N,`<span class="code-link" data-filename="${x.filename}">${x.filename}</span>`)}),M.replace(/@@MATH_BLOCK_(\d+)@@/g,(x,h)=>{const N=m[parseInt(h)];return N.type==="display"?`\\[ ${N.content.trim()} \\]`:`\\( ${N.content.trim()} \\)`})}else{const s=a.examples||[];let m=`
