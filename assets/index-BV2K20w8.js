...
`),R([]);try{await J(u),$.start("run-code"),await Ye(a);const d=u.includes("matplotlib.widgets")||u.includes("Slider")||u.includes("Button");ie(d);const b=d?"module://matplotlib_pyodide.wasm_backend":"AGG";if(Y!==b&&(await te(a,d),K(b)),await a.runPythonAsync(`
import sys
...
`:"")+M),Q(!1),$.end("run-code");return}const x=await a.runPythonAsync("sys.stdout.getvalue()");if(g(x||"執行完成（無文字內容輸出 ）"),V)await Je(a);else{const k=await Ke(a);R(k)}}catch(d){const b=ae(d);g(b),console.error(d)}finally{Q(!1),$.end("run-code")}}},me=async u=>{let d=u;if(u&&!u.content&&(d=await de(u.id)),!!d&&(y(d),E(null),D(""),g(""),R([]),d&&d.examples&&a)){const b=d.examples.map(x=>x.code).join(`
