...
• 在除法前檢查除數是否為零
• 使用 if 語句避免除以零
• 檢查資料中是否有零值`},{pattern:/ImportError:?\s*(.*)/,format:s=>`❌ 匯入錯誤${s[1]?"："+s[1].trim():""}
...
• 檢查是否支援該模組
• 重新整理頁面後再試一次
• 查看瀏覽器主控台 (F12) 了解完整錯誤訊息`}];for(const{pattern:s,format:r}of t){const o=e.match(s);if(o)return r(o)}return`❌ 執行錯誤
