...
- **數學公式渲染**：採用 KaTeX **Auto-render (後處理)** 模式與專有的 **冪等復原 (Idempotent Recovery)** 技術。透過待 Markdown 轉換為 HTML 後的自動後置掃描，結合能自動導正損壞反斜線的正則處理器，徹底解決了語法衝突與資料轉義損壞問題，提供出版等級的金融模型顯示效果。
